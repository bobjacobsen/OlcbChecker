...
def check():
    ...
    # Send a search Identify Producer with search nibbles 0x2F.FF.FF and 
    # not- Allocate, Exact, Address Only, Any/Default protocol 0x60.
    message = Message(MTI.Producer_Consumer_Event_Report , NodeID(olcbchecker.ownnodeid()), destination,
                [0x09,0x00,0x99,0xFF,   0x2F, 0xFF, 0xFF, 0x60])
    olcbchecker.sendMessage(message)
    ...
    try :
        ...
        while True :
            ...
            if received.mti is MTI.Producer_Identified_Valid :
                eventID = EventID(received.data)
                if eventID != [0x09,0x00,0x99,0xFF,   0x2F, 0xFF, 0xFF, 0x60] :
                    logger.warning ("Failure - PCER without Producer Identified: {}".format(eventID))
                    return(3)
                else :
                    break # all OK
    ...
    # Send a search Identify Producer with search nibbles 0x1F.FF.FF and 
    # not- Allocate, Exact, Address Only, Any/Default protocol 0x60.
    message = Message(MTI.Producer_Consumer_Event_Report , NodeID(olcbchecker.ownnodeid()), destination,
                [0x09,0x00,0x99,0xFF,   0x1F, 0xFF, 0xFF, 0x60])
    olcbchecker.sendMessage(message)
    ...
    try :
        ...
        while True :
            ...
            if received.mti is MTI.Producer_Identified_Valid :
                eventID = EventID(received.data)
                if eventID != [0x09,0x00,0x99,0xFF,   0x1F, 0xFF, 0xFF, 0x60] :
                    logger.warning ("Failure - PCER without Producer Identified: {}".format(eventID))
                    return(3)
                else :
                    break # all OK
