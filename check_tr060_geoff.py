...
'''
Check global emergency off

Usage:
python3.10 check_tr060_geoff.py 
...
def check():
    ...
    try :  # to be sure to set speed to zero
        ...
        # produce global Emergency Off event
        message = Message(MTI.Producer_Consumer_Event_Report, NodeID(olcbchecker.ownnodeid()), destination, [0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF])
        olcbchecker.sendMessage(message)
        # no reply expected to this
        ...
        # produce global clear Emergency OFF event
        message = Message(MTI.Producer_Consumer_Event_Report, NodeID(olcbchecker.ownnodeid()), destination, [0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFE])
        olcbchecker.sendMessage(message)
        # no reply expected to this
