...
\subsection{Listener Configuration checking}

This checks the Listener Attach, Detach, and Query lifecycle
defined in section 6.5 of the Train Control Protocol Standard.

A cleanup block detaches all listeners regardless of test outcome.

\begin{enumerate}

    \item Query the listener list. Expect count = 0 (no listeners attached).

    \item Attach Listener~A with the REVERSE flag set. Expect an OK result.

    \item Query the listener list. Expect count = 1. Verify Listener~A's
          Node~ID and the REVERSE flag are returned.

    \item Attach Listener~B with the LINK\_F0 flag set. Expect an OK result.

    \item Query index~0. Expect Listener~A with REVERSE.

    \item Query index~1. Expect Listener~B with LINK\_F0.

    \item Detach Listener~A. Expect an OK result.

    \item Query the listener list. Expect count = 1 with Listener~B remaining.

    \item Detach Listener~A again (already detached). Expect a failure result.

    \item Detach Listener~B. Expect an OK result.

    \item Query the listener list. Expect count = 0.
