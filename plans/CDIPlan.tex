...
This checks against the Configuration Description Information Standard section 5.1.2,
and the Memory Configuration Standard section 4.14.

This is a comprehensive ACDI conformance check covering consistency between PIP, Configuration
Options, Address Space Information, SNIP, and the ACDI address spaces (0xFC and 0xFB).

\begin{enumerate}

\item PIP check — Verifies ACDI and Memory Configuration are both present in PIP.

\item Configuration Options flags — Reads the Options reply and checks that the
...
\item Address Space Info — For each ACDI space flagged as readable, queries Address
...
\item SNIP comparison — Reads all fields from 0xFC (manufacturer, model, hardware version,
software version) and 0xFB (user name, user description) and compares each against the
...
\item Writeback roundtrip — If User Write is flagged, saves the current user name
from 0xFB address 1, writes the string "OLCB\_TST", reads it back and
verifies, then restores the original value.

\item Rejects write to read-only 0xFC — Attempts a write to the manufacturer
space and verifies the node rejects it (either Datagram Rejected or Write Reply
Fail).

\item Rejects write to 0xFB version byte — Attempts a write to 0xFB address 0
